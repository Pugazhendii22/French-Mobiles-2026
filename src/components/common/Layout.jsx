...
const NAV_ITEMS = [
  ...
  {
    label: 'Import Contacts', path: '/admin/import-contacts', adminOnly: true,
    icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 4h2a2 2 0 012 2v14a2 2 0 01-2 2H6a2 2 0 01-2-2V6a2 2 0 012-2h2m2 0h4M9 14l2 2 4-4" /></svg>,
  },
  {
...
/* ─────────────────────────────────────────────
   BOTTOM TAB BAR
   Four destinations plus a raised Scan action in the middle,
   the arrangement people already expect from a native app.
────────────────────────────────────────────── */
const BOTTOM_NAV = [
  {
  ...
  {
    label: 'Mobiles', path: '/inventory/second-hand',
    icon: (active) => (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={active ? '2.5' : '1.8'} d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z" />
      </svg>
    ),
  },
  { scanAction: true, label: 'Scan', path: '/scanner' },
  {
    label: 'Service', path: '/service',
    icon: (active) => (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const SidebarContent = ({ navItems, location, userName, currentUser, userRole, initials, onClose, onLogout }) => (
  <div className="flex flex-col h-full bg-slate-900 text-white pt-safe pb-safe">
    {/* Logo */}
...
const Layout = ({
  ...
  // Every screen opens at the top, the way a native push transition behaves
  useEffect(() => {
    window.scrollTo(0, 0);
  }, [location.pathname]);
  ...
  // Modals cover the screen at z-50 but the tab bar and FAB sit at z-40, so they
  // used to show through the dimmed backdrop - a stray "+" floating over the form.
  // Every modal locks body scroll, so watch that and stand the chrome down.
  useEffect(() => {
    const sync = () => setOverlayOpen(document.body.style.overflow === 'hidden');
  ...
  // Reserve room for the fixed bottom bar(s) including the home-indicator inset
  const mobilePb = hasStickyAction ? 'pb-app-nav-action' : 'pb-app-nav';
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex">
      ...
      >
        ...
        {/* ── APP BAR ── */}
        <header className="bg-[#002395] sticky top-0 z-30 print:hidden pt-safe shadow-md">
          <div className="flex items-center h-14 px-2 gap-1">
            ...
            {showBackButton ? (
              <button
                onClick={handleBack}
                id="layout-back-btn"
                className="w-10 h-10 flex items-center justify-center text-white rounded-full active:bg-white/20"
                aria-label="Go back"
              >
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <button
                ...
                className="w-10 h-10 flex items-center justify-center text-white rounded-full active:bg-white/20 md:hidden"
            ...
            {/* Title - centred on mobile like a native title bar, left on desktop */}
            <h2 className="flex-1 text-[17px] font-semibold text-white truncate text-center md:text-left md:pl-2">
              {title || 'French Mobiles'}
            </h2>

            <div className="flex items-center gap-2 shrink-0">
              <span className="hidden md:block text-sm text-white/80 font-medium truncate max-w-[150px]">
                {userName || currentUser?.email}
              </span>
              <button
                onClick={() => setMenuDrawerOpen(true)}
                className="w-9 h-9 rounded-full bg-white text-[#002395] flex items-center justify-center text-sm font-bold shrink-0"
                aria-label="Open menu"
              >
                {initials}
              </button>
        ...
        <main className={`flex-1 ${mobilePb}`}>
          <div
            ...
            className={`
              ...
              ${pageType === 'form' ? 'max-w-2xl mx-auto px-4 py-5' : 'p-4 md:p-6'}
            `}
          >
            {children}
          </div>
        ...
        {hasStickyAction && (
          <div className="md:hidden fixed bottom-app-nav left-0 right-0 z-30 bg-white border-t border-gray-200 px-4 py-3 shadow-lg print:hidden break-words">
            {stickyAction}
          </div>
        )}
        ...
        <nav
          ...
          className={`md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-gray-200 pb-safe print:hidden ${
            overlayOpen ? 'hidden' : ''
          }`}
        >
          <div className="flex items-stretch h-16">
            {BOTTOM_NAV.map((item) => {
              ...
              /* Raised centre action - the shop's most repeated gesture */
              if (item.scanAction) {
                return (
                  <div key={item.label} className="flex-1 flex flex-col items-center justify-start">
                    <button
                      id="bottom-nav-scan"
                      onClick={() => navigate(item.path)}
                      className={`-mt-6 w-14 h-14 rounded-full flex items-center justify-center border-4 border-white shadow-lg ${
                        isActive ? 'bg-[#001a6e]' : 'bg-[#002395]'
                      }`}
                    ...
                    <span className={`text-[10px] font-semibold mt-1 ${isActive ? 'text-[#002395]' : 'text-gray-400'}`}>
                      {item.label}
                    </span>
                  </div>
                );
              ...
              return (
                ...
                >
                  {/* active pill above the icon, the usual tab-bar affordance */}
                  <span
                    className={`absolute top-0 h-1 w-8 rounded-b-full transition-colors ${
                      isActive ? 'bg-[#002395]' : 'bg-transparent'
                    }`}
                  />
                  <span className={isActive ? 'text-[#002395]' : 'text-gray-400'}>
                    {item.icon(isActive)}
                  </span>
      ...
      {fab && !overlayOpen && (
        <div className="md:hidden fixed bottom-app-fab right-4 z-40 print:hidden">
          {fab}
        </div>
      )}
    </div>
