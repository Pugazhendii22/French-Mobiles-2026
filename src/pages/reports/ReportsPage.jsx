...
const ReportsPage = () => {
  ...
  let payCash = 0; let payUpi = 0; let payCard = 0; let paySplit = 0; let payFinance = 0;
  ...
  filteredSales.forEach(s => {
    ...
    // Anything else is a finance provider - without this the revenue would be
    // missing from the payment breakdown entirely.
    else payFinance += Number(s.totalAmount) || 0;
            ...
            <div id="section-Sales" className={activeTab !== 'Sales' ? 'hidden' : ''}>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-[#e2e8f0] p-4 md:p-6 mb-4">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="w-16 text-sm text-[#64748b] flex-shrink-0">Finance</span>
                    <div className="flex-1 bg-[#f8fafc] rounded-full h-3"><div className="bg-indigo-600 h-3 rounded-full" style={{ width: `${totalSalesRev ? (payFinance / totalSalesRev) * 100 : 0}%` }}></div></div>
                    <span className="text-sm font-bold text-[#0f172a] w-8 text-right flex-shrink-0">{payFinance ? Math.round(payFinance / 1000) : 0}</span>
                  </div>
                </div>
              </div>
